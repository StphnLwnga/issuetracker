		...
		test('Update an issue with an invalid _id', function (done) {
			chai
				...
				.end(function (err, res) {
					...
					assert.equal(res.body._id, '5ac2389cae0720dfb23b2b3b');
					assert.property(res.body, 'error');
	...
	suite('Delete an issue', function () {
		...
		test('DELETE issue with an invalid _id', function (done) {
			chai
				...
				.delete(endpoint)
				.send({
					_id: '5ac2389cab3420dfb23b2b3b',
				})
				.end(function (err, res) {
					assert.equal(res.status, 200);
					assert.include(res.headers['content-type'], 'application/json');
					// console.log(res.body._id)
					assert.equal(res.body._id, '5ac2389cab3420dfb23b2b3b');
					assert.property(res.body, 'error');
